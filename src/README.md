...
* Step 2: Start from the first planner of selected planners, customzie the problems size for processing data.
  ...
  * **Start from first planner in list: PRP**
    ...
    3 5-10 12 14
    ```
 * Step 3: Customize the specific size of problems. (* will loop Step 3 for  selected domains: 3, 5-10, 12, 14, then move to Step 4*)
 ...
 * Step 4: Decide to use same problems size as the previous planner or not.
 
   * 1: Will apply the previous problems size to remainging planners.(*will loop Step 5 for remaining planners*)
   * 2: Go to Step 2 again with next planner after Step 5.
      ...
      Do you want to use the same range for remaining planners?
        ...
        1
      ```  
  * Step 5: Save processed data into a csv file. (*For now, only have one method: mean, so the data will be stored in `results/corresponding-planner-folder/mean`*)
 
   	* 1:  Enter a name to save the selected domains for testing in `.csv` file. (*name should exclude ".csv"*)
  	 * Will automatically collect data(Step background) then 
  	   * start from Step 2 again: if choose 2 in Step 4
  	   * loop Step 5: if choose 1 in Step 4
  	 * If not find the key word, it will log on console: ERROR: Not find the key word {file_path}
        ```
